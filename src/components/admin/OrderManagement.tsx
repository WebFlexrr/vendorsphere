...
const OrderManagement = () => {
  ...
  // Mock order data
  const orders = [
    { id: 'ORD-7452', customer: 'John Doe', date: '2023-06-18', total: 124.95, items: 3, status: 'Delivered', payment: 'Paid' },
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="all">
                  ...
                  <tbody>
                    {orders.map((order) => (
                      <tr key={order.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 text-center">
                          <Badge variant="outline" className={order.payment === 'Paid' ? 'bg-green-50 text-green-700' : 
        ...
        {['pending', 'processing', 'shipped', 'delivered'].map((tab) => (
          <TabsContent key={tab} value={tab}>
            <Card>
              <CardContent>
                <p className="py-4 text-gray-500">
                  {tab.charAt(0).toUpperCase() + tab.slice(1)} orders would be displayed here.
                </p>
              </CardContent>
            </Card>
          </TabsContent>
